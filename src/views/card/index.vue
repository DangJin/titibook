<template>
  <div class="" id="card">
    ...
    <el-table :data="list" row-key="id" border fit element-loading-text="给我一点时间"
      ...
      <el-table-column width="300" align="center" label="类型名称">
        <template slot-scope="scope">
          <span>{{ scope.row.name }}</span>
        </template>
      </el-table-column>
      <el-table-column align="center" label="会员权益">
        <template slot-scope="scope">
          <span>{{scope.row.description}}</span>
        </template>
      </el-table-column>
      <el-table-column align="center" label="会员期限">
        <template slot-scope="scope">
          <span> {{scope.row.day}} 天</span>
        </template>
      </el-table-column>
      <el-table-column align="center" label="兑换门槛">
        <template slot-scope="scope">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="modifyVisible">
      <el-form ref="dataForm" :model="addData" label-position="left" label-width="100px" :rules="rules"
        ...
        <el-form-item label="会员期限" prop="day">
          <el-input v-model="addData.day" type="number" min="1"></el-input>
        </el-form-item>
        <el-form-item label="兑换门槛" prop="point">
          <el-input v-model="addData.point" type="number" min="0.01"></el-input>
        </el-form-item>
      </el-form>
...
//          0删除状态1草稿状态2发布状态
    ...
    methods: {
       ...
       */
      getList (refresh) {
        cardList(this.params).then(response => {
